...
def pdfreq(
    df, tables=None, weight=None, order="internal", converters=None, maxlevels=20
):
    ...
    while tables:
        ...
        pdfreq_(
            v1=df[v1].apply(converters[v1])
            if converters and v1 in converters
            else df[v1],
            v2=None
            if v2 is None
            else (
                df[v2].apply(converters[v2])
                if converters and v2 in converters
                else df[v2]
            ),
...
def pdfreq_(v1, v2=None, weight=None, order="internal", maxlevels=20):
    ...
    if v2 is not None:
        ...
        if order == "freq":
            cnt.iloc[:-1, :] = (
                cnt.iloc[:-1, :].sort_values("All", ascending=False).values
            )
            cnt = cnt.transpose()
            cnt.iloc[:-1, :] = (
                cnt.iloc[:-1, :].sort_values("All", ascending=False).values
        ...
        print(
            header_tmplt.format(
                cnt.columns.name, *list(cnt.columns), w=col_width, w1=col1_width
            )
        )
        print(
            header_tmplt.format(
                cnt.index.name, *([" "] * ncol), w=col_width, w1=col1_width
            )
        )
        print("-" * ((col_width + 2) * ncol + col1_width + 4))
        for i in range(nrow):
            print(
                cnt_tmplt.format(
                    cnt.index[i], *list(cnt.iloc[i, :]), w=col_width, w1=col1_width
                )
            )
            print(
                pct_tmplt.format(" ", *list(pct.iloc[i, :]), w=col_width, w1=col1_width)
            )
            if i == 0:
                print(
                    rowcol_pct_tmplt.format(
                        "Row Pct",
                        *list(row_pct.iloc[i, :-1]),
                        " ",
                        w=col_width,
                        w1=col1_width,
                    )
...
def pdcompare(df1, df2, keys=None):
    ...
    if keys is None:
        ...
        merged = df1.merge(
            df2, on=keys, how="outer", suffixes=("_left", "_right"), indicator=True
        )
    ...
    num_cols = set(df1.select_dtypes(include=[np.number]).columns).intersection(
        set(cols.loc[cols._merge.eq("both"), "col"])
    )
    if keys is not None:
    ...
    mindex = pd.MultiIndex.from_tuples(
        [(t[0], t[2]) for t in (c.rpartition("_") for c in diff_df_num.columns)],
        names=("var", "diff_type"),
    )
    ...
    cat_cols = set(df1.select_dtypes(exclude=[np.number]).columns).intersection(
        set(cols.loc[cols._merge.eq("both"), "col"])
    )
    if keys is not None:
    ...
    for c in list(cat_cols):
        diff_prop = (
            merged[c + "_left"].eq(merged[c + "_right"]).value_counts(normalize=True)
        )
        diff_prop.name = c
...
def summary(df, outfile="summary.csv"):
    ...
    with open(outfile, "w") as f:
        ...
        for c in df.columns:
            ...
            freq = pd.DataFrame({"cnt": cnt, "pct": cnt / cnt.sum()})
